...
def run(
    ...
    # Print results
    # Sửa định dạng in: Loại bỏ seen (Images)
    pf = "%22s" + "%11i" + "%11.3g" * 4  # Chỉ giữ Instances, P, R, mAP50, mAP50-95
    ...
    if (verbose or (nc < 50 and not training)) and nc > 1 and len(stats):
        for i, c in enumerate(ap_class):
            # Sửa định dạng in cho từng lớp: Loại bỏ seen
